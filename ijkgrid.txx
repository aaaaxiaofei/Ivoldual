...
namespace IJK {
  ...
  public:
    ...
    /// Return true if vertex_index is the index of a grid vertex.
    ...
    /// Return true if cube_index is the index of a grid cube.
  ...
  protected:
    ...
    /// iv0+cube_vertex_increment[k] k'th vertex of cube 
  ...
  public:
    ...
    /// Return the direction of a grid edge.
  ...
  */
  ...
  /// Compute increment to add to index of a vertex to get next vertices 
  ///   along the axes.
  /// @param dimension  Dimension of grid.
  /// @param axis_size  Array: <em>axis_size[d]</em> = 
  ///    Number of vertices along axis \a d.
  ...
  template <typename STYPE, typename GRID_TYPE>
  template <typename GTYPE2>
  const GRID_SPACING<STYPE, GRID_TYPE> &
  GRID_SPACING<STYPE, GRID_TYPE>::Copy(const GTYPE2 & right)
  {
    if (&right != this) {         // avoid self-assignment
      FreeLocal();
      GTYPE2::Copy(right);
