...
namespace IJK {
  ...
  protected:
    ...
    /// Copy (x0,x1,...,xd) in scalar_grid to (p[0]*x0,p[1]*x1,...,p[d]*xd) 
  ...
  public:
    ...
    const SCALAR_GRID_ALLOC & Copy(const GTYPE2 & right);

    /// Copy assignment.
    template <typename GTYPE2>
    const SCALAR_GRID_ALLOC & operator = (const GTYPE2 & right)
    { return(Copy(right)); }
  ...
  public:
    ...
    (const DTYPE2 dimension, const ATYPE2 * axis_size);
    template <typename DTYPE2, typename ATYPE2, 
              typename VTYPE2, typename NTYPE2>
    void SetSize       ///< Set dimensions and axis size.
    (const GRID<DTYPE2,ATYPE2,VTYPE2,NTYPE2> & grid2);
  ...
  */
  ...
  SCALAR_GRID_ALLOC<BASE_CLASS>::Copy(const GTYPE2 & right)
  {
    if (&right != this) {
      this->SetSize(right);
      this->CopyScalar(right);
    }
  ...
  (const GCLASS & scalar_grid, const PTYPE subsample_period[])
  {
    ...
    this->SetSize(dimension, subsampled_axis_size.PtrConst());
  ...
  (const GTYPE & scalar_grid2, const PTYPE supersample_period[])
  {
    ...
    for (DTYPE d = 0; d < dimension; d++) {
      supersampled_axis_size[d] =
        compute_supersample_size
        (scalar_grid2.AxisSize(d), supersample_period[d]);
    }
  ...
  (const GTYPE & scalar_grid2, const PTYPE supersample_period)
  {
    ...
    NTYPE numv0;
    compute_num_vertices_in_grid_facet(scalar_grid2, 0, numv0);
  ...
  (const GTYPE & scalar_grid2, const PTYPE supersample_period[])
  {
    ...
    subsample_subgrid_vertices
      (*this, 0, subgrid_axis_size.PtrConst(), supersample_period, 
       vlist1.Ptr());

    for (VTYPE x0 = 0; x0 < scalar_grid2.AxisSize(0); x0++) {
      VTYPE x1 = x0*supersample_period[0];
      for (VTYPE i = 0; i < numv0; i++) {
        VTYPE v0 = vlist0[i]+x0;
        VTYPE v1 = vlist1[i]+x1;
        STYPE s = STYPE(scalar_grid2.Scalar(v0));
  ...
  (const PTYPE supersample_period[])
  {
    ...
    for (DTYPE d = 0; d < this->Dimension(); d++) {
      for (DTYPE j = 0; j < d; j++) { subsample_period[j] = 1; };
      for (DTYPE j = d; j < this->Dimension(); j++)
        { subsample_period[j] = supersample_period[j]; };
      for (DTYPE j = 0; j < this->Dimension(); j++)
        { subgrid_axis_size[j] = this->AxisSize(j); };
      subgrid_axis_size[d] = 1;
      ...
      for (VTYPE x = 0; x+1 < this->AxisSize(d); x += supersample_period[d]) {

        VTYPE inc0 = x*axis_increment[d];
        VTYPE inc1 = inc0 + supersample_period[d]*axis_increment[d];
        for (VTYPE i = 0; i < numv; i++) {
          VTYPE v0 = vlist[i] + inc0;
          VTYPE v1 = vlist[i] + inc1;

          VTYPE v2 = v0;
          for (VTYPE j = 1; j < supersample_period[d]; j++) {
            v2 += axis_increment[d];
            STYPE s0 = this->scalar[v0];
            STYPE s1 = this->scalar[v1];
            this->scalar[v2] =
              linear_interpolate(s0, 0, s1, supersample_period[d], j);
          }
        }
      }
    }
   ...
   const ATYPE region_edge_length)
  {
    ...
    this->SetSize(dimension, num_regions_along_axis.PtrConst());
  ...
  /// Returns true if all adjacent vertices are greater than or equal to s.
  /// @param scalar_grid Scalar grid.
  /// @param iv Vertex index.
  /// @pre Vertex iv must be an internal grid vertex.
  /// @param s Scalar value.
  template <typename GRID_TYPE, typename VTYPE, typename STYPE>
  bool are_all_adjacent_vertices_ge
  (const GRID_TYPE & scalar_grid, const VTYPE iv, const STYPE s)
  {
    typedef typename GRID_TYPE::DIMENSION_TYPE DTYPE;
    ...
    for (DTYPE d = 0; d < dimension; d++) {
      ...
      const VTYPE iv_next = scalar_grid.NextVertex(iv, d);
      if (scalar_grid.Scalar(iv_next) < s) { return(false); }

      const VTYPE iv_prev = scalar_grid.PrevVertex(iv, d);
      if (scalar_grid.Scalar(iv_prev) < s) { return(false); }
    }

    return(true);
  }

  /// Returns true if all adjacent vertices are less than s.
  /// @param scalar_grid Scalar grid.
  /// @param iv Vertex index.
  /// @pre Vertex iv must be an internal grid vertex.
  /// @param s Scalar value.
  template <typename GRID_TYPE, typename VTYPE, typename STYPE>
  bool are_all_adjacent_vertices_lt
  (const GRID_TYPE & scalar_grid, const VTYPE iv, const STYPE s)
  {
    typedef typename GRID_TYPE::DIMENSION_TYPE DTYPE;
  ...
  /// Returns true if all incident edges are bipolar.
  /// @param scalar_grid Scalar grid.
  /// @param iv Vertex index.
  /// @pre Vertex iv must be an internal grid vertex.
  /// @param s Scalar value.
  template <typename GRID_TYPE, typename VTYPE, typename STYPE>
  bool are_all_incident_edges_bipolar
  (const GRID_TYPE & scalar_grid, const VTYPE iv, const STYPE s)
  {
    if (scalar_grid.Scalar(iv) < s) {
      return(are_all_adjacent_vertices_ge(scalar_grid, iv, s));
    }
    else {
      return(are_all_adjacent_vertices_lt(scalar_grid, iv, s));
    }
  }
   ...
   const SCALAR_GRID_BASE<GRID_CLASS,STYPE> & scalar_grid)
  {
    ...
    if (scalar_grid.Dimension() <= 1) {
      ...
      compute_num_vertices_in_grid_ridge
        (dimension, axis_size, DTYPE(0), DTYPE(1), num_vertices_in_ridge);
      IJK::ARRAY<VTYPE> vlist(num_vertices_in_ridge);
      get_vertices_in_grid_ridge
        (dimension, axis_size, DTYPE(0), DTYPE(1), vlist.Ptr());
