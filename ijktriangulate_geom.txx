...
namespace IJK {
  ...
  /*!
   *  Compute the cosine of the smallest triangle angle
   *    in the triangulation of a quadrilateral into four triangles.
   *  - Quadrilateral is split into four triangles formed from coord4[]
  ...
  template <typename DTYPE, 
            typename CTYPE0, typename CTYPE1, typename CTYPE2, 
            typename CTYPE3, typename CTYPE4, typename COS_TYPE,
            typename MTYPE>
   ...
   const MTYPE max_small_magnitude, COS_TYPE & cos_min_angle, bool & flag_zero)
  {
    ...
    compute_cos_min_triangle_angle
      (dimension, coord0, coord1, coord4, max_small_magnitude, 
       cos[0], flag_tri_zero[0]);
    compute_cos_min_triangle_angle
      (dimension, coord1, coord2, coord4, max_small_magnitude, 
       cos[1], flag_tri_zero[1]);
    compute_cos_min_triangle_angle
      (dimension, coord2, coord3, coord4, max_small_magnitude, 
       cos[2], flag_tri_zero[2]);
    compute_cos_min_triangle_angle
      (dimension, coord3, coord0, coord4, max_small_magnitude, 
   ...
   std::vector<VTYPE1> & tri_vert)
  {
    ...
    CTYPE cos_min_diag02, cos_min_diag13;
    ...
    if (flag_zero_diag13 || (cos_min_diag02 < cos_min_diag13)) 
   ...
   std::vector<VTYPE2> & tri_vert)
  {
    ...
    CTYPE cos_min_diag02, cos_min_diag13, cos_min_tri4;
    bool flag_zero_diag02, flag_zero_diag13, flag_zero_tri4;
    ...
    if (flag_zero_tri4) { 
      ...
      if (flag_zero_diag13 || (cos_min_tri4 < cos_min_diag13))
        { flag_tri4 = true; }
      else {
        flag_tri4 = false;
        flag_diag02 = false;
      ...
      if (flag_zero_diag02 || (cos_min_tri4 < cos_min_diag02))
        { flag_tri4 = true; }
      else {
        flag_tri4 = false;
        flag_diag02 = true;
      ...
      if (cos_min_diag02 < cos_min_diag13) {
        flag_diag02 = true;
